        ...
        <div className='fixed top-0 bg-[white] z-[20]'>
            <div className='w-[100vw] min-h-[80px]  border-b-[1px] border-[#dcdcdc] px-[20px] flex items-center justify-between md:px-[40px] '>
                ...
                <div className='flex items-center justify-center    gap-[10px] relative'>
                    {userData && userData.role === 'admin' && <span className='text-[18px] cursor-pointer rounded-[50px] hover:bg-[#ded9d9] px-[8px] py-[5px] hidden md:block' onClick={()=>navigate("/listingpage1")}>List your property</span>}
                    ...
                    {showpopup && <div className='w-[240px] min-h-[280px] absolute bg-white top-[110%] right-[3%] border-[1px] border-[#dcdcdc] z-10 rounded-xl shadow-xl md:right-[10%]'>
                        <ul className='w-[100%] h-[100%] text-[16px] flex items-start justify-start flex-col py-[8px]'>
                            {!userData && <li className='w-[100%] px-[20px] py-[12px] hover:bg-[#f7f7f7] cursor-pointer font-medium transition' onClick={()=>{navigate("/login");setShowpopup(false)}}>Login</li>}
                            {!userData && <li className='w-[100%] px-[20px] py-[12px] hover:bg-[#f7f7f7] cursor-pointer font-medium transition' onClick={()=>{navigate("/signup");setShowpopup(false)}}>Sign Up</li>}
                            {userData && <li className='w-[100%] px-[20px] py-[12px] hover:bg-[#f7f7f7] cursor-pointer font-semibold transition' onClick={()=>{
                            ...
                            {userData && <div className='w-[100%] h-[1px] bg-[#e5e5e5]'></div>}
                            {userData && userData.role === 'admin' && <li className='w-[100%] px-[20px] py-[12px] hover:bg-[#f7f7f7] cursor-pointer transition' onClick={()=>{navigate("/listingpage1");setShowpopup(false)}}>Create Listing</li>}
                            {userData && userData.role === 'admin' && <li className='w-[100%] px-[20px] py-[12px] hover:bg-[#f7f7f7] cursor-pointer transition'onClick={()=>{navigate("/mylisting");setShowpopup(false)}}>Manage Listings</li>}
                            {userData && userData.role !== 'admin' && <li className='w-[100%] px-[20px] py-[12px] hover:bg-[#f7f7f7] cursor-pointer transition'  onClick={()=>{navigate("/mybooking");setShowpopup(false)}}>My Bookings</li>}
                            {userData && <div className='w-[100%] h-[1px] bg-[#e5e5e5]'></div>}
                            {userData && <li className='w-[100%] px-[20px] py-[12px] hover:bg-[#f7f7f7] cursor-pointer text-red-600 font-medium transition' onClick={()=>{handleLogOut();setShowpopup(false)}}>Logout</li>}
                        </ul>
