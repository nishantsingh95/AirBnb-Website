...
function ListingPage1() {
  ...
  let { title, setTitle,
    ...
    ratings, setRatings } = useContext(listingDataContext)
    ...
    <div className='w-[100%] h-[100vh] bg-white flex items-center justify-center relative overflow-auto'>
      ...
      <form action="" className='max-w-[900px] w-[90%] h-[550px] flex items-center justify-start flex-col md:items-start gap-[10px] overflow-auto mt-[50px]' onSubmit={(e) => {
        ...
        <div className='w-[90%] flex items-start justify-start flex-col gap-[10px]'>
          ...
          <input type="number" id='quantity' min="1" className='w-[90%] h-[40px] border-[2px] border-[#555656] rounded-lg text-[18px] px-[20px]' required onChange={(e) => setTotalQuantity(parseInt(e.target.value))} value={totalQuantity} placeholder='How many units available?' />
          <p className='text-sm text-gray-600'>Specify how many identical properties/units are available for booking</p>
        </div>

        <div className='w-[90%] flex items-start justify-start flex-col gap-[10px]'>
          <label htmlFor="ratings" className='text-[20px]'>Rating</label>
          <input type="number" id='ratings' min="0" max="5" step="0.1" className='w-[90%] h-[40px] border-[2px] border-[#555656] rounded-lg text-[18px] px-[20px]' required onChange={(e) => setRatings(parseFloat(e.target.value))} value={ratings} placeholder='Rating (0-5)' />
          <p className='text-sm text-gray-600'>Give your property an initial rating (e.g., 4.5)</p>
        </div>
